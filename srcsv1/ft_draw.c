...
/*   Updated: 2019/11/24 18:04:31 by adda-sil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	int x;
	// void *img = mlx_new_image(game->mlx, game->win.width, game->win.height);
	// int bits, size_line, endian;
	// char *data = mlx_get_data_addr(img, &bits, &size_line, &endian);
	// printf("Img data: Bits %d, SLine %d, Endian %d\n", bits, size_line, endian);
	...
	while (++x < game->win.width)
    {
      ...
      //texturing calculations
		...
		for(int y = drawStart; y < drawEnd; y++)
		{
			...
			// data[y * game->win.height + y] = color <<= 3;
		}
		while (drawEnd++ <= game->win.height)
			// data[x * drawEnd] = game->env.FLOOR;
			mlx_pixel_put(game->mlx, game->win.ref, x, drawEnd, game->env.FLOOR);
		// mlx_put_image_to_window(game->mlx, game->win.ref, img, 0, 0);
	}
	return (1);
}
