...
/*   Updated: 2019/12/01 04:33:09 by adda-sil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_healthbar(t_game *game)
{
	...
	draw = (t_ipos){ .x = pos.x, .y = pos.y };
	double ratio = (double)game->player.hp / (double)(game->player.max_hp);
	int	hp = HEALTHBAR_WIDTH * ratio - 1;
	int color;
	if (ratio > .75)
		color = 0x0000ff00;
	else
		color = ratio > 0.5 ? 0x00ffff00 : 0x00ff0000;
