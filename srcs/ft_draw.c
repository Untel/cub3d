...
int	draw_frame(t_game *game)
{
	...
	pos.x = -1;
	while (++pos.x < game->win.width)
    {
		//calculate ray position and direction
		double camera_x = 2 * pos.x / (double)(game->win.width - 1); //x-coordinate in camera space
		...
		{
			pos.y = t;
			ft_draw_renderer(game, pos, game->env.CEIL);
		}
		while(drawStart++ < drawEnd)
		{
			...
			ft_draw_renderer(game, pos, color);
		}
		while (drawEnd++ <= game->win.height)
		{
			pos.y = drawEnd;
			ft_draw_renderer(game, pos, game->env.FLOOR);
		}
	}
	return (1);
}
